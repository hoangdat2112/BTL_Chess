...
public class ChessGame_AI {
    ...
    public void setPieceAt(Square square, ChessPiece piece) {
    ...
    private static Square findKingPosition(Player player) {
        for (ChessPiece piece : piecesBox) {
            if (piece.getPlayer() == player && piece.getChessman() == Chessman.KING) {
                return new Square(piece.getCol(), piece.getRow());
            }
        }
        return null;
    }

    public static boolean isKingInCheck(Player player) {
        Square kingPosition = findKingPosition(player);
        if (kingPosition == null) return false;
        Log.d("CMNR", String.valueOf(piecesBox.size()));
        for (ChessPiece piece : piecesBox) {
            if (piece.getPlayer() != player) {
                Square attackerPosition = new Square(piece.getCol(), piece.getRow());
                if (canMove(attackerPosition, kingPosition)) {
                    return true;
                }
            }
        }
    ...
    public static boolean moveWouldExposeCheck(Square from, Square to, Player player) {
        ...
        if (movingPiece != null) {
            ...
            piecesBox.add(new ChessPiece(to.getCol(), to.getRow(),
                    movingPiece.getPlayer(), movingPiece.getChessman(),
                    movingPiece.getResID()));

            boolean wouldBeInCheck = isKingInCheck(player);

            piecesBox.remove(pieceAt(to));
            piecesBox.add(movingPiece);
            if (capturedPiece != null) {
                piecesBox.add(capturedPiece);
            }
    ...
    private static boolean canKnightMove(Square from, Square to) {
        if(pieceAt(to)!=null&&pieceAt(to).getPlayer().isWhite()==pieceAt(from).getPlayer().isWhite()) return false;
    ...
    public static boolean canMove(Square from, Square to) {
        ...
        boolean basicMove = false;
        switch (movingPiece.getChessman()) {
            case KNIGHT:
                basicMove = canKnightMove(from, to);
                break;
            case ROOK:
                basicMove = canRookMove(from, to);
                break;
            case BISHOP:
                basicMove = canBishopMove(from, to);
                break;
            case QUEEN:
                basicMove = canQueenMove(from, to);
                break;
            case KING:
                basicMove = canKingMove(from, to);
                break;
            case PAWN:
                basicMove = canPawnMove(from, to);
                break;
        }

        if (!basicMove) return false;

        return !moveWouldExposeCheck(from, to, movingPiece.getPlayer());
    }

    public static void movePiece(Square from, Square to) {
        ChessPiece movingPiece = pieceAt(from);
        if (movingPiece == null || movingPiece.getPlayer() != currentPlayer) {
            return;
        }
        ...
        if (canMove(from, to)) {
            ...
            if (isKingInCheck(currentPlayer)) {
                System.out.println("Check! " + currentPlayer + " king is in check!");
            }
        }
    }
        ...
        for (int col = 0; col < 8; col++) {
            ...
            if (piece != null) {
                ...
                switch (piece.getChessman()) {
                    case KING:
                        desc.append(white ? "k" : "K");
                        break;
                    case QUEEN:
                        desc.append(white ? "q" : "Q");
                        break;
                    case BISHOP:
                        desc.append(white ? "b" : "B");
                        break;
                    case ROOK:
                        desc.append(white ? "r" : "R");
                        break;
                    case KNIGHT:
                        desc.append(white ? "n" : "N");
                        break;
                    case PAWN:
                        desc.append(white ? "p" : "P");
                        break;
                }
            } else {
                desc.append(".");
    ...
    public static Set<ChessPiece> getPiecesBox() {
        return piecesBox;
    }

    public static void setPiecesBox(Set<ChessPiece> piecesBox) {
        ChessGame_AI.piecesBox = piecesBox;
    }
